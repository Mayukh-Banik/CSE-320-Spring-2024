...
int dtmf_generate(FILE *events_in, FILE *audio_out, uint32_t length) {
	...
	int prevStart = starting;
	int prevEndin = ending;
	if (prevStart > prevEndin)
	{
		return EOF;
	}
	...
	for (int i = 0; i < length; i++) {
		...
		if (i > ending - 1) {
			...
			if (a != EOF) {
				...
				if (starting < prevStart) {return EOF;}
				if (starting < prevEndin) {return EOF;}
				if (ending < prevStart) {return EOF;}
				if (ending < prevEndin) {return EOF;}
				if (starting > ending) {return EOF;}
				if (ending > length) {return EOF;}
				
				if(starting == prevEndin) 
				{
					double a = cos(2.0 * pi * row_freq * i / audio_frame_rate);
					double b = cos(2.0 * pi * col_freq * i / audio_frame_rate);
					double c = a * 0.5 + b * 0.5 ;
		...
		int dtmf_int = dtmf;
		fputc((dtmf_int >> 8) & 0xFF, audio_out);
		fputc(dtmf_int& 0xFF, audio_out);
